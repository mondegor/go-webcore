...
- `mrserver.HeaderKeyRealIP` (`X-Real-Ip`) и `mrserver.HeaderKeyForwardedFor`
  (`X-Forwarded-For`) - прокси-заголовки с IP-адресом клиента. Первый содержит
  одиночный адрес, второй - цепочку адресов через запятую;

### Changed
- у `parser.ClientIP` изменён список заголовков по умолчанию, по которым определяется
  прокси-адрес клиента: вместо `X-Client-IP`, `X-Cluster-Client-IP`, `X-Forwarded-For`
  просматриваются `X-Real-Ip`, `X-Forwarded-For`. `X-Real-Ip` идёт первым, т.к. содержит
  одиночный адрес и не требует разбора цепочки. **ВНИМАНИЕ**: оба заголовка достоверны
  только тогда, когда запрос гарантированно приходит через доверенный прокси, - клиент
  вправе прислать их сам, и парсер этого не проверяет;

## 2026-07-23
### Added
- `mrserver.HeaderKeyInternalLangCode` (`X-Internal-Lang-Code`) и
  `mrserver.HeaderKeyInternalTimeZone` (`X-Internal-Time-Zone`) - внутренние заголовки
  с языком и часовым поясом из профиля пользователя. Заполняются
  `middleware.CheckAccessHandler` и срезаются `middleware.CheckAccessTokenHandler`.
  Содержат одиночное значение и принимаются только при точном совпадении со списком
  приложения; приоритетнее клиентских заголовков, но уступают query-параметрам;
- `mrserver.HeaderKeyAcceptTimeZone` (`X-Accept-Time-Zone`) - клиентский заголовок
  с предпочтительным часовым поясом. Формат: `Europe/Moscow;offset=+03:00;dst=0`, где
  первый сегмент без `=` - IANA-имя, а `offset` и `dst` - запасной путь на случай, когда
  имя приложением не зарегистрировано: по ним подбирается ближайший по смещению из его
  поясов (`LocationList.NameByOffset`). Любая часть необязательна, но пробелы в значении
  не допускаются нигде - ни вокруг сегментов, ни вокруг `=`;
- `request.ParserTimeZone` и его реализация `parser.TimeZone` - определяют часовой пояс
  запроса независимо от авторизации. Имена разбираются через список предзагруженных
  поясов приложения (`util/timezone.LocationList` из `go-core`);
- `request.ParserUser` получил методы `LangCode` и `TimeZoneName` - сырые значения
  внутренних заголовков `X-Internal-Lang-Code` и `X-Internal-Time-Zone`, то есть язык
  и часовой пояс **из профиля** пользователя без применения приоритетов и без сверки
  со списками приложения. Нужны, когда требуется именно профильное значение (например,
  показать сохранённую настройку или заметить, что она не задана); итоговые язык и пояс
  запроса по-прежнему дают `ParserLocale` и `ParserTimeZone`. На гостевых маршрутах,
  где внутренние заголовки срезаются, оба метода возвращают пустую строку;

### Changed
- внутренние заголовки получили общий префикс `HeaderKeyInternal*`, чтобы внутреннее
  отличалось от клиентского по имени константы, а не по памяти:
  `mrserver.HeaderKeyUserIDSlashGroup` -> `mrserver.HeaderKeyInternalUserIDSlashGroup`,
  `mrserver.HeaderKeySessionID` -> `mrserver.HeaderKeyInternalSessionID`.
  Значения заголовков те же, поэтому правка сводится к переименованию;
- `middleware.CheckAccessHandler` больше не перезаписывает и не удаляет клиентские
  `Accept-Language` и `X-Accept-Time-Zone`: значения из профиля кладутся во внутренние заголовки,
  то есть сервер не перекрывает клиента, а добавляет источник более высокого приоритета.
  **ВНИМАНИЕ**: у пользователя с незаполненным профилем теперь действуют значения клиента,
  а не значения по умолчанию, - раньше клиентские заголовки в этом случае удалялись;
- `middleware.CheckAccessTokenHandler` срезает `X-Internal-Lang-Code`
  и `X-Internal-Time-Zone` наравне с `X-Internal-UserId-Group` и `X-Internal-Session-Id`:
  иначе клиент задавал бы язык и пояс "от имени сервера", минуя подбор. Клиентские
  `Accept-Language` и `X-Accept-Time-Zone` не трогаются - задать их клиент вправе
  на любом маршруте, в том числе гостевом;
- `parser.Locale` и `parser.TimeZone` получили третий уровень источников (см. схему выше).
  Первые два строгие: значение принимается только при точном совпадении со списком приложения,
  поэтому `?lang=fr-CH` при списке `["ru","en","fr"]` больше не сводится к `fr`, а параметры
  `offset`/`dst` во внутреннем заголовке пояса не разбираются. Разбор клиентских заголовков
  не изменился и остаётся терпимым: по ним по-прежнему подбирается ближайшее поддерживаемое значение;

## 2026-07-20
### Changed
- `parser.Locale` берёт параметр языка только из строки запроса
  (`r.URL.Query()`) вместо `r.FormValue()`. Ранее для `form-encoded` запросов параметр
  подхватывался из тела, при этом `ParseForm` поглощал `r.Body` и обработчик получал
  пустое тело. Передача языка в теле запроса больше не поддерживается;
- `parser.Locale` больше не отбрасывает заголовок `Accept-Language` целиком из-за одного
  негодного элемента: языки уцелевших элементов извлекаются и учитываются по весу `q`.
  Поэлементно разбираются первые 8 элементов заголовка, остальные отбрасываются;
- Поле `Time` в ответах об ошибках (`mrresp`) формируется в формате `RFC3339`
  с точностью до миллисекунд (например: `2020-01-01T12:00:00.000Z`) вместо `time.RFC3339`;
...
## 2025-09-28 (v0.29.1)
### Changed
- Проведены доработки под новую версию библиотеки `go-core`;

### Removed
- Перенесены некоторые пакеты в проект `go-core`;

## 2025-09-25 (v0.29.0)
### Changed
- Обновлена большая часть пакетов библиотеки;
...
### Changed
- Изменена логика управления ошибками с учётом обновлений библиотеки `go-core`.
...
## 2024-06-15
### Changed
- Обновлена система формирования ошибок в связи с внедрением новой версии библиотеки `go-core`:
