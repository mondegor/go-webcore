...
- Добавлена ошибка `FactoryErrHttpMultipartFormFile` используемая, когда возникли проблемы при загрузке файла на сервер;
- При описании пользовательских ошибок (в yaml файлах на разных языках), можно теперь использовать следующие переменные: `name`, `type`, `value`, `param`;
