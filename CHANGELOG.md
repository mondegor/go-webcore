...
## 2026-07-20
### Added
- `mrserver.HeaderKeyTimeZone` (`X-Internal-Time-Zone`) - внутренний заголовок с часовым поясом
  пользователя (IANA-имя). Заполняется `middleware.CheckAccessHandler` из профиля пользователя
  (`mraccess.User.TimeZone()`) и удаляется `middleware.CheckAccessTokenHandler`
  для неавторизованных запросов;
- `request.ParserUser.Location()` - возвращает часовой пояс пользователя из запроса.
  Реализация `parser.User` разбирает имя не сама, а через список предзагруженных поясов
  (`util/timezone.LocationList` из `go-core`), поэтому к базе часовых поясов в процессе
  обработки запроса обращений нет. Незарегистрированное имя логируется и сводится к `time.UTC`;

### Changed
- в интерфейс `request.ParserUser` добавлен метод `Location()` -
  внешние реализации интерфейса требуют доработки;
- `parser.NewUser()` принимает список часовых поясов первым аргументом:
  `NewUser(logger)` -> `NewUser(locations, logger)`. При `locations = nil` часовой пояс
  запроса всегда определяется как `time.UTC`;
...
- `parser.Locale` больше не отбрасывает заголовок `Accept-Language` целиком из-за одного
  ...
  Поэлементно разбираются первые 16 элементов заголовка, остальные отбрасываются;
