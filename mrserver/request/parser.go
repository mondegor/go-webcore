...
type (
	...
	//  1. query-параметр разовой смены языка (например: ?lang);
	//  2. заголовок X-Internal-Lang-Code - язык из профиля пользователя, который
	//     на авторизованных маршрутах подставляет middleware;
	//  3. заголовок Accept-Language - предпочтения клиента;
	//  4. язык по умолчанию.
	//
	// Первые два источника строгие: значение принимается, только если оно точно совпадает
	// с языком из списка, зарегистрированного приложением. Третий разбирается терпимее -
	// по нему подбирается ближайший поддерживаемый язык.
	//
	// Авторизации не требует: язык определяется и для гостевых запросов.
	ParserLocale interface {
		Language(r *http.Request) string
		Localizer(r *http.Request) mrcore.Localizer
	}

	// ParserTimeZone - парсер для определения часового пояса запроса.
	//
	// Источники в порядке приоритета:
	//  1. query-параметр разового переопределения пояса (например: ?tz);
	//  2. заголовок X-Internal-Time-Zone - пояс из профиля пользователя, который
	//     на авторизованных маршрутах подставляет middleware;
	//  3. заголовок X-Accept-Time-Zone - пояс клиента;
	//  4. пояс по умолчанию.
	//
	// Первые два источника строгие: значение принимается, только если оно точно совпадает
	// с именем из списка, зарегистрированного приложением. Третий разбирается терпимее -
	// незарегистрированное имя не отбрасывает заголовок целиком, а передаёт разбор
	// параметрам offset и dst, по которым подбирается ближайший по смещению пояс.
	//
	// Авторизации не требует: часовой пояс определяется и для гостевых запросов.
	ParserTimeZone interface {
		Location(r *http.Request) *time.Location
	}

	// ParserUser - парсер для получения информации о пользователе из запроса.
	// Извлекает данные из внутренних заголовков, установленных middleware.
	//
	// LangCode и TimeZoneName возвращают значения из профиля пользователя как есть.
	// Итоговые язык и часовой пояс запроса дают не эти методы, а ParserLocale
	// и ParserTimeZone.
	ParserUser interface {
		...
		LangCode(r *http.Request) string
		TimeZoneName(r *http.Request) string
	}
)
