...
func (p *User) UserID(r *http.Request) uuid.UUID {
	val, _, _ := strings.Cut(r.Header.Get(mrserver.HeaderKeyInternalUserIDSlashGroup), "/") // отбрасывается группа пользователя
	if val == "" {
		return uuid.Nil
	}
...
func (p *User) UserAndGroup(r *http.Request) (userID uuid.UUID, group string) {
	val, group, ok := strings.Cut(r.Header.Get(mrserver.HeaderKeyInternalUserIDSlashGroup), "/")
	if !ok && val == "" {
		return uuid.Nil, ""
	}
...
func (p *User) SessionID(r *http.Request) string {
	return r.Header.Get(mrserver.HeaderKeyInternalSessionID)
}

// LangCode - извлекает код языка пользователя из HTTP запроса.
func (p *User) LangCode(r *http.Request) string {
	return r.Header.Get(mrserver.HeaderKeyInternalLangCode)
}

// TimeZoneName - извлекает имя часового пояса пользователя из HTTP запроса.
func (p *User) TimeZoneName(r *http.Request) string {
	return r.Header.Get(mrserver.HeaderKeyInternalTimeZone)
}
