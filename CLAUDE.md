...
This module is tightly coupled to the sibling module `github.com/mondegor/go-sysmess`: several interface packages that used to live here (`mraccess`, `mrworker`, `mrrun`, `mridempotency`, the `Logger`/`mrtrace` types, the `mrstorage` interfaces) now live in **go-sysmess**, and `go-webcore` keeps only the adapters that wire them to concrete libraries (Prometheus, chi, slog, etc.). When you can't find an interface here, it is almost certainly imported from `go-sysmess`. There is a commented-out `replace` directive in `go.mod` for local development against a sibling checkout.
...
- **mrstorage** — storage adapters; `mrprometheus/` holds `DBCollector`, a Prometheus collector that exports DB connection-pool stats via `go-sysmess`'s `mrstorage.DBStatProvider`.
- **mrlog** — logging adapters. Currently `slog/sentry/` (a Sentry handler for `slog`). The primary `Logger`/`mrtrace` types live in `go-sysmess`.
